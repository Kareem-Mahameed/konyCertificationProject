...
function getCategories(selectedCatID){
	...
	if (!kony.string.equalsIgnoreCase(selectedCatID, "none")){
      ...
      var operationName = mobileFabricConfiguration.integrationServices[0].operations[0];
      var inputParams={"key":mobileFabricConfiguration.bestByAPIKey,"categoryID":selectedCatID};
      ...
      else if (mobileFabricConfiguration.isKonySDKObjectInitialized)
      {
          getServiceResponse(operationName,inputParams,"getting Best By Categories",getCategoriesSuccessCallback);
			...
			// The user didn't pick a value so we'll show the alert
		    kony.ui.Alert({ message: "Please select a valid cat type",alertType:constants. ALERT_TYPE_INFO, alertTitle:"Categories",yesLabel:"OK"}, {});
	}
}
...
function getCategoriesSuccessCallback (getCategoriesResponse)
{
  ...
  if (getCategoriesResponse !== null && getCategoriesResponse.opstatus === 0){
    ...
    if (getCategoriesResponse.categoryID !=="" && getCategoriesResponse.subCategories !== null && getCategoriesResponse.subCategories !== "")
        ...
        if (getCategoriesResponse.subCategories.length >0)
          {
            ...
            frmHome.sgmtCategories.widgetDataMap={catId:"subCategoryID",lblCatname:"subCategoryName"};
      ...
      {
        categoriesStack.pop();
        kony.ui.Alert({ message: "error in calling the service ",alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Sub Categories",yesLabel:"OK"}, {});
      }
    kony.application.dismissLoadingScreen();
    ...
    kony.ui.Alert({ message: "Service call failed with opstatus " + getCategoriesResponse.opstatus,alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Categories List",yesLabel:"OK"}, {});
  }
}
...
function getSubCategory()
{
    ...
    categoriesStack.push({breadCrumb:frmHome.lblBreadCrumb.text,categories:frmHome.sgmtCategories.data});
    ...
    kony.print("########## categoriesStack: " + JSON.stringify(categoriesStack));
  	...
  	// ######		ALL SPA CHANNELS
  	//#ifdef spaan
	//#define SPASpecificCode_account1
	//#endif
	//#ifdef spaip
	//#define SPASpecificCode_account1
	//#endif
	...
	//#ifdef iphone
	//#define nativeSpecificCode_account1
	//#endif
	//#ifdef android
	//#define nativeSpecificCode_account1
	//#endif
	...
	//#ifdef SPASpecificCode_account1
 	selData = frmHome.sgmtCategories.selectedItems[0];
	//#endif

	//#ifndef SPASpecificCode_account1
 	selData = frmHome.sgmtCategories.selectedRowItems[0];
	//#endif 
...
function getCatProducts(category){
	...
	if (!kony.string.equalsIgnoreCase(category.categoryID, "none")){
      ...
      var inputParams={"key":mobileFabricConfiguration.bestByAPIKey,"categoryID":category.categoryID,"pageNo":category.pageNo};
      ...
      else if (mobileFabricConfiguration.isKonySDKObjectInitialized)
      {
          getServiceResponse(operationName,inputParams,"getting Category Products",getCatProductsSuccessCallback);
			...
			// The user didn't pick a value so we'll show the alert
		    kony.ui.Alert({ message: "Please select a valid cat ",alertType:constants. ALERT_TYPE_INFO, alertTitle:"Categories",yesLabel:"OK"}, {});
	}
}
...
function getCatProductsSuccessCallback (getCatProductsResponse)
{
  ...
  if (getCatProductsResponse !== null && getCatProductsResponse.opstatus === 0){
    ...
    if (getCatProductsResponse.products !== null && getCatProductsResponse.products !== "")
        ...
        if (getCatProductsResponse.products.length >0)
          {
            ...
            frmProductList.sgmtProducts.widgetDataMap={productID:"productSKU",lblProductName:"productName",lblPrice:"regularPrice",lblRating:"customerReviewAverage",imgRowThumb:"mediumImage"};
          ...
          {
        	kony.ui.Alert({ message: "Empty product list: "+getCatProductsResponse.products,alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Empty Products List",yesLabel:"OK"}, {}); 
            frmHome.show();
          }
      }
    else
      {
        categoriesStack.pop();
        kony.ui.Alert({ message: "Empty product list",alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Products List not available",yesLabel:"OK"}, {});
      }
    kony.application.dismissLoadingScreen();
	...
	kony.application.dismissLoadingScreen();
    // The call failed because opstatus was not 0 so we'll alert the user and show that opststus
    kony.ui.Alert({ message: "Service call failed with opstatus " + getCatProductsResponse.opstatus,alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Products List",yesLabel:"OK"}, {});
  }
}
  ...
  if (searchRecord.searchText!=="" && searchRecord.searchText!==null && searchRecord.searchText.trim().length>0)
    {
      ...
      {
    	kony.ui.Alert({ message: "Please search for valid keywords other than: !@#$%^&*()_+-=[]{};':\|,.<>?`~ ",alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Products Search",yesLabel:"OK"}, {});
      	frmHome.flxSearch.txtSearch.setFocus(true);
  	  }
    }
  else 
    {
      kony.ui.Alert({ message: "Please enter search keywords",alertType:constants. ALERT_TYPE_ERROR, alertTitle:"Products Search",yesLabel:"OK"}, {});
      frmHome.flxSearch.txtSearch.setFocus(true);
    }
...
function showProductDetails()
{
	...
	//#ifndef SPASpecificCode_account1
 	productSelData = frmProductList.sgmtProducts.selectedRowItems[0];
	//#endif
...
function getProductReviews(productSKU)
{
	...
	if (!kony.string.equalsIgnoreCase(productSKU, "none")){
      ...
      var inputParams={"key":mobileFabricConfiguration.bestByAPIKey,"productSKU":productSKU};
      ...
      else if (mobileFabricConfiguration.isKonySDKObjectInitialized)
      {
          getServiceResponse(operationName,inputParams,"getting Product Reviews",getProductReviewsSuccessCallback);
			...
			// The user didn't pick a value so we'll show the alert
		    kony.ui.Alert({ message: "Please select a valid Product ",alertType:constants. ALERT_TYPE_INFO, alertTitle:"Product Reviews",yesLabel:"OK"}, {});
	}
}
...
function getProductReviewsSuccessCallback (getProductReviewsResponse)
{
  ...
  if (getProductReviewsResponse !== null && getProductReviewsResponse.opstatus === 0)
  {
        ...
        if (getProductReviewsResponse.reviews.length >0)
          {
            ...
            frmProductDetails.sgmtReview.widgetDataMap={lblReviewTitle:"title",lblReviewerName:"reviewerName",lblReviewContent:"comment",imgRating:"rating"};
	...
	kony.application.dismissLoadingScreen();
    // The call failed because opstatus was not 0 so we'll alert the user and show that opststus
    kony.ui.Alert({ message: "Service call failed with opstatus " + getProductReviewsResponse.opstatus,alertType:constants. ALERT_TYPE_ERROR, alertTitle:"ProductReviewsResponse",yesLabel:"OK"}, {});
  }
  frmProductDetails.show();
}
